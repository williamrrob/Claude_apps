...
// "source" nodes (e.g. the Latin lemma conferre) are label-only — they have no
// dictionary entry by design, so they're excluded from the record-existence check.
const sourceWords = new Set((doc.placements || []).filter((p) => p.kind === "source").map((p) => p.w));
const collapse = doc.collapse || {};
const covered = new Set(treeWords.concat(Object.keys(collapse)));

// --- validation ---
const dangling = [...covered].filter((w) => !vocab.has(w) && !sourceWords.has(w));
